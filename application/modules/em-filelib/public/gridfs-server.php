...
try {
    $application->getBootstrap()->bootstrap('customer')->bootstrap('mongo')->bootstrap('filelib');
    
    $filelib = $application->getBootstrap()->getResource('filelib');

    $gridfs = $filelib->getStorage()->getGridFs();
    
    $uri = substr($_SERVER['REQUEST_URI'], strlen($filelib->getPublicDirectoryPrefix()) + 1);
    

    $file = $gridfs->findOne(array('filename' => $uri));

    header("Content-type: {$file->file['metadata']['mimetype']}");
    echo $file->getBytes();
